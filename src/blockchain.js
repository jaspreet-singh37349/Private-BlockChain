...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            {
                let prevTime = parseInt(message.split(':')[1])
                //let prevMin = new Date()
                let currTime = parseInt(new Date().getTime().toString().slice(0,-3))
                //let currMin = new Date(currTime).getMinutes()
                //console.log(currTime - prevTime)
                if(currTime-prevTime<=5*60)
                {
                    ...
                    reject({error:"Your signature is not correct."})
                }
                else
                {
                    reject({error:"Time elapsed is more than 5 minutes."})
                }
            }
            
            reject({error:"message format not correct"})
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
        let block = self.chain.filter((blockk)=>{
               return blockk.hash===hash
           })
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            resolve("No error! Chain is valid")
        });
    }
